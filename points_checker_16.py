...
UPDATE_INTERVAL_SECONDS = 295  # BIG API only updates every 60 seconds 
...
#        dict(name="Yoshiro", id=2343315859, clan="RFIL", last_points=0, current_points=0, total_points_earned=0, consecutive_zeros=0),
        dict(name="Luker", id=2467377557, clan="fr3e", last_points=0, current_points=0, total_points_earned=0, consecutive_zeros=0),
...
def fetch_user_points(user):
    response = subprocess.run(["curl", "--silent", f"{API_URL}/{user['clan']}"], capture_output=True, text=True)
...
def print_user_data(user, current_points_str, points_difference, max_points_length):
    spaces_name = alignment_spaces(user['name'])
...
def fetch_and_display_user_data():
    ...
    for user in data['users']:
        ...
        # Add a 50 ms pause after each subprocess
        time.sleep(0.05)
